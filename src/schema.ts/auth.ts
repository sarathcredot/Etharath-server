...
export const UserRegisterSchema = z.object({
    ...
    role: z.enum(['vendor', 'retailer', 'sales_executive'])
        .refine(
            (val) => ['vendor', 'retailer', 'sales_executive'].includes(val),
...
export const UpdateUserSchema = z.object({
    userName: z.string().nonempty("userName is required").min(1, "Name is required"),
    email: z.string().nonempty("Email is required").email("Invalid email format"),
    phoneNumber: z.string().nonempty("phoneNumber is required").min(10, "Phone number must be at least 10 characters").max(15, "Phone number must be at most 15 characters"),
